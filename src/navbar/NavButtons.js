...
const NavButtons = ({history, auth}) => {
  return(
    <div className='flex'>
      <button onClick={ev => history.push('/')} className="ml-4 px-3 py-2 rounded-md text-sm font-medium leading-5 text-gray-300 hover:text-white hover:bg-gray-700 focus:outline-none focus:text-white focus:bg-gray-700 transition duration-150 ease-in-out">Find Jobs</button>
      {auth.id && (
